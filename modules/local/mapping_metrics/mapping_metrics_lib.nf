process MAPPING_METRICS_LIB {
    tag "$meta.id"
    label 'process_mapping_metrics_lib'
    ...
    ## Convert filtered_reads (individual mapped reads) to a fragment count before comparing
    ## against raw_reads_pairs (fragments), so unmerged pairs aren't double-counted: each mapped
    ## pair contributes 2 to filtered_reads but is only 1 fragment, so subtract half the paired count
