...
include { ADAPTERREMOVAL    as FP_ADAPTERREMOVAL    }   from '../../../modules/local/adapterremoval/main.nf'
...
workflow FASTQ_PROCESSING {
    ...
    if (params.adapter_trimming_tool == 'fastp') {
        ...
        // Run AdapterRemoval3 (default) for adapter trimming and read merging
        FP_ADAPTERREMOVAL ( raw_reads )
        ch_versions  = ch_versions.mix(FP_ADAPTERREMOVAL.out.versions)
        ch_trim_json = FP_ADAPTERREMOVAL.out.json
        ch_trim_log  = FP_ADAPTERREMOVAL.out.log

        // If merge_reads set to 'true', update meta.single_end to true for the merged reads output
        if (params.merge_reads.toBoolean()) {
            ch_merged_reads = FP_ADAPTERREMOVAL.out.reads_merged.map { meta, reads ->
                def new_meta = meta.clone()
                new_meta.single_end = true
                [new_meta, reads]
            }
        ...
        // Note: AdaptClean is fastp-JSON-specific (it reads fastp's adapter_cutting/total_cycles
        // fields) and AdapterRemoval3 is itself a dedicated, mature adapter-trimming tool, so
        // AdaptClean is not applicable/needed here regardless of params.adaptclean
        if (params.keep_unmerged_reads.toBoolean()) {
            def ch_unmerged_reads = FP_ADAPTERREMOVAL.out.reads_unmerged.map { meta, reads ->
                def new_meta = meta.clone()
                new_meta.id = meta.id + "-unmerged"
                [new_meta, reads]
            }
            // Singleton reads (one mate discarded, the other kept) are leftover, imperfectly
            // paired output just like unmerged reads, so gate them behind the same param
            def ch_singleton_reads = FP_ADAPTERREMOVAL.out.reads_singleton.map { meta, reads ->
                def new_meta = meta.clone()
                new_meta.id = meta.id + "-singleton"
                new_meta.single_end = true
                [new_meta, reads]
            }
            ch_adapter_removed_reads = ch_adapter_removed_reads
                .mix(FP_ADAPTERREMOVAL.out.reads_se)
                .mix(FP_ADAPTERREMOVAL.out.reads_pe)
                .mix(ch_unmerged_reads)
                .mix(ch_singleton_reads)
        }
        else {
            ch_adapter_removed_reads = ch_adapter_removed_reads
                .mix(FP_ADAPTERREMOVAL.out.reads_se)
    ...
    fastp_json          = ch_trim_json                                                                              // channel: [ val(meta), [ json ] ] (fastp or adapterremoval, depending on adapter_trimming_tool)
    fastp_log           = ch_trim_log                                                                               // channel: [ val(meta), [ log ] ]  (fastp or adapterremoval, depending on adapter_trimming_tool)
