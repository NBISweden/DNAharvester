...
def get_identity_distribution_and_avg_mapq(bam_file):
    identity_values, edit_distances, read_lengths = [], [], []
    total_mapq = []
    total_nb_reads = 0
    ...
    return identity_values, edit_distances, read_lengths, total_nb_reads, avg_mapq, avg_identity

def evenness_data(bam_file):
    depth_data = list(pysam.depth('-a', bam_file, split_lines=True))
    ...
    for i in range(len(tiles)-1):
        df_tile = df[(df['Position']>=tiles[i]) & (df['Position']<tiles[i+1])]
        fraction_covered = np.sum(df_tile['Depth']>0)/len(df_tile) if len(df_tile)>0 else 0
        if fraction_covered>0:
            tile_covered_count += 1
        boc.extend([fraction_covered]*len(df_tile))

    if len(boc)<len(df):
        boc.extend([boc[-1]]*(len(df)-len(boc)))
    elif len(boc)>len(df):
        boc = boc[:len(df)]
    df['boc'] = boc
    df['Position2'] = df.index
    percent_tiles_covered = round(tile_covered_count/(len(tiles)-1)*100,2)
    return df, mean_coverage, percent_bases_covered, percent_tiles_covered
...
def process_bam_file(bam_file, pdf_pages, stats_row=None):
    ...
    df_even, mean_coverage, percent_bases_covered, percent_tiles_covered = evenness_data(bam_file)
    CT,CpG,other = calculate_dna_damage(bam_file)

    fig = plt.figure(figsize=(22,20))
    gs = gridspec.GridSpec(3,2, height_ratios=[1,1,1.5])

    # Super-title with reference
    ...
    grouped = df_even.groupby(np.arange(len(df_even))//(len(df_even)//500+1))
    for _,g in grouped:
        ax3.plot(g['Position2'], g['boc'], color='black', linewidth=1)
    ...
    # Row3: DNA damage left, Table right
    ax_damage = plt.subplot(gs[2,0])
    x = list(range(31))
    ax_damage.plot(x, CT,label="C to T",color="#d62728",linewidth=3)  # thicker red line
    ...
    ax_table.axis('off')
    if stats_row is not None:
        metrics = ['n_reads','mapping_quality','edit_distances','read_ani_mean','coverage_mean',
                   'breadth_exp_ratio','site_density','norm_entropy','norm_gini']
        ...
        table.scale(1,3)  # slightly smaller to avoid overlap

        # bold headers and color header row
...
def main():
    ...
    tab = tab[tab['n_reads']>=100]
    tab = tab[tab['breadth_exp_ratio']>=0.90]
